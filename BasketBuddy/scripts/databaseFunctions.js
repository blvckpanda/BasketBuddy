...
import {
  ...
  confirmEditModal,
  deleteAllItemButton,
  emptyListPageContainer,
  allItemContainer,
  userGuideContainer,
  confirmDeleteModalMessage,
  confirmDeleteModal,
  closeConfirmDeleteModalButton,
  confirmDeleteModalButton,
  closeConfirmEditModalButton,
...
// database setup
const db = new Dexie("BusketBuddy");
...
export const fetchItems = async () => {
  ...
  document.querySelector(
    ".total__price"
  ).innerHTML = `Total: KSH ${allItems.reduce(
    (total, item) => total + item.price * item.quantity,
    0
  )}`;

  allItemContainer.innerHTML = allItems.map(
    (item, i) => `
    <div
      class="single__item ${item.isPurchased && "purchased__single"}"
      onContextMenu="editItemOnRightClick(event, ${item.id}, '${item.name}', '${
      item.quantity
    }', '${item.price}', '${item.isPurchased}')"
    >
      <label>
        <div class="single__item__container"
          onmousedown="toggleItemStatus(event, ${item.id}, ${item.isPurchased})"
          ontouchstart="editItem(event, ${item.id}, '${item.name}', '${
      item.quantity
    }', '${item.price}', '${item.isPurchased}')"
...
window.editItemOnRightClick = async (
  e,
  id,
  name,
  quantity,
